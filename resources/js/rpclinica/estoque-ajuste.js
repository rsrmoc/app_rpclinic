    ...
    deleteEntradaProduto(indice) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (this.produtosEntrada[indice].cd_ajuste_prod) {
                    axios.post(`/rpclinica/estoque-ajuste-prod-delete/${this.produtosEntrada[indice].cd_ajuste_prod}`)
                        .then((res) => {
                            toastr['success']('Ajuste do produto excluída com sucesso!')
                            this.produtosEntrada.splice(indice, 1)
                        })
                        .catch((err) => toastr['error']('Houve um erro ao excluir a ajuste do produto.'))
                    return;
                }
