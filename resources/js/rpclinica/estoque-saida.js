    ...
    deleteEntradaProduto(indice) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (this.produtosEntrada[indice].cd_solic_saida) {
                    axios.post(`/rpclinica/estoque-saida-prod-delete/${this.produtosEntrada[indice].cd_solic_saida}`)
                        .then((res) => {
                            toastr['success']('Saída do produto excluída com sucesso!')
                            this.produtosEntrada.splice(indice, 1)
                        })
                        .catch((err) => toastr['error']('Houve um erro ao excluir a saída do produto.'))
                    return;
                }
