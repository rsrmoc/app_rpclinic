...
$parametroOperador = [
    ...
    'in' => 'Na Lista [ in ]',
    'not_in' => 'Não esta na lista[ not in ]',
    'is_null' => 'Vazio [ is null ]',
    'is_not_null' => 'Não Vazio [ is not null ]',
];
...
@section('content')
    ...
    <div id="main-wrapper" x-data="edit_relatorios">
        <div class="col-md-12 ">
                ...
                <div class="panel-body">
                    ...
                    <form role="form" id="addUser" method="post"  role="form">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-md-offset-1">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" required="" name="titulo" value="{{ $relatorio->titulo }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" required="" name="area">
                                        ...
                                        <option value="CO" {{ $relatorio->area == 'CO' ? 'selected' : '' }}>Consultório</option>
                                        <option value="FI" {{ $relatorio->area == 'FI' ? 'selected' : '' }}>Financeiro</option>
                                        <option value="ES" {{ $relatorio->area == 'ES' ? 'selected' : '' }}>Estoque</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select x-on:change="getConteudoView()" class="form-control" required="" name="conteudo">
                                        ...
                                        @foreach ($views as $view)
                                            <option value="{{ $view->view }}" {{ $relatorio->conteudo == $view->view ? 'selected' : '' }}>{{ $view->nome }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" required="" name="tipo_relatorio">
                                        <option value="">Selecione</option>
                                        <option value="REL" {{ $relatorio->tipo_relatorio == 'REL' ? 'selected' : '' }}>Relatório HTML</option>
                                        <option value="XLS" {{ $relatorio->tipo_relatorio == 'XLS' ? 'selected' : '' }}>Relatório XLS</option>
                                        <option value="PDF" {{ $relatorio->tipo_relatorio == 'PDF' ? 'selected' : '' }}>Relatório PDF</option>
                                        <option value="GCOL" {{ $relatorio->tipo_relatorio == 'GCOL' ? 'selected' : '' }}>Grafico Coluna</option>
                                        <option value="GPIZ" {{ $relatorio->tipo_relatorio == 'GPIZ' ? 'selected' : '' }}>Grafico Pizza</option>
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" required="" name="layout">
                                        <option value="">Selecione</option>
                                        <option value="R" {{ $relatorio->layout == 'R' ? 'selected' : '' }}>Retrato</option>
                                        <option value="P" {{ $relatorio->layout == 'P' ? 'selected' : '' }}>Paissagem</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select class="form-control"  name="restricao">
                                        <option value="">Selecione</option>
                                        <option value="PROF" {{ $relatorio->restricao == 'PROF' ? 'selected' : '' }}>Usuario Logado [ Profisional ]</option> 
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="box-footer">
                            <input type="button" class="btn btn-success" value="Editar" x-on:click="editRelatorio()" />
                            <input type="button" class="btn btn-danger" value="Excluir" x-on:click="deleteRelatorio()" />
                        </div>
                    </form>
                </div>
            </div>
...
@section('scripts')
    <script>
        const relatorios = @js($relatorio);
        const relatorio_campos = @js($campos);
        const relatorio_parametros = @js($parametros);
        const relatorio_calculos = @js($calculos);
        const relatorio_ordem = @js($ordenar);
    </script>
    <script src="{{ asset('js/rpclinica/relatorios-edit.js') }}"></script>
@endsection
