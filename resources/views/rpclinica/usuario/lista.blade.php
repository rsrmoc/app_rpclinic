...
@section('content')
    <div class="page-title">
        ...
        <div class="page-breadcrumb">
            <ol class="breadcrumb">
                <li><a href="{{ route('usuario.listar') }}">Relação</a></li>
            </ol>
        </div>
    </div>
    <div id="main-wrapper">
                        ...
                        <table class="display table dataTable table-striped">
                            ...
                            <tbody>
                                @foreach ($usuarios as $usuario)
                                    <tr id="user-{{ $usuario->cd_usuario }}">
                                        <td>{{ $usuario->email }}</td>
                                        <td>{{ $usuario->nm_usuario }}</td>
                                        <td>{{ $usuario->perfil?->nm_perfil }}</td>
                                        <td>{{ $usuario->profissional?->nm_profissional }}</td>
                                        <td > <span class="{{ CSS_SN_ATIVO[$usuario->sn_ativo] }}">{{ SN_ATIVO[$usuario->sn_ativo] }} </span></td>
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <a href="{{ route('usuario.edit', ['usuario' => $usuario->cd_usuario]) }}" class="btn btn-success">
                                                    <i class="fa fa-edit"></i>
                                                </a>

                                                <button onclick="delete_cadastro('{{ route('usuario.delete', ['usuario' => $usuario->cd_usuario]) }}', '#user-{{ $usuario->cd_usuario }}')"
