...
@section('content')
    <div class="page-title">
        <h3>Editar Ajuste de Estoque</h3>
        ...
        <div class="col-md-12 ">
            <div class="panel panel-white">
                <div class="panel-body">
                    ...
                    <form role="form" action="{{ route('estoque.ajuste.update', ['ajuste' => $ajuste->cd_ajuste]) }}" method="post" x-ref="formAjuste"
                                    ...
                                    <select class=" form-control" tabindex="-1" style=" width: 100%"
                                        ...
                                        @foreach ($estoques as $estoque)
                                            <option value="{{ $estoque->cd_estoque }}"
                                                @if ($ajuste->cd_estoque == $estoque->cd_estoque) selected @endif>
                                                {{ $estoque->nm_estoque }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class=" form-control" tabindex="-1" style=" width: 100%"
                                        ...
                                        @foreach ($tiposAjuste as $tipoAjuste)
                                            <option value="{{ $tipoAjuste->cd_tipo_ajuste }}"
                                                @if ($ajuste->cd_tipo_ajuste == $tipoAjuste->cd_tipo_ajuste) selected @endif>
                                                {{ $tipoAjuste->nm_tipo_ajuste }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select class=" form-control" tabindex="-1" style=" width: 100%"
                                        ...
                                        @foreach ($setores as $setor)
                                            <option value="{{ $setor->cd_setor }}"
                                                @if ($ajuste->cd_setor == $setor->cd_setor) selected @endif>
                                                {{ $setor->nm_setor }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-2 ">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="exampleInputEmail1"
                                        ...
                                        value="{{ $ajuste->nr_doc }}" required />
                                </div>
                            </div>
                        </div>

                        <template x-for="produto, indice in produtosEntrada">
                            <template x-if="!produto.cd_ajuste">
                                <div>
                        ...
                        <tbody>
                            <template x-for="produto, indice in produtosEntrada">
                                <tr>
                                    ...
                                    <td x-html="nomeDoLote(produto.cd_lote_prod)"></td>
...
@section('scripts')
    ...
    <script>
        ...
        const ajusteProdutos = @js($ajuste->ajustesProdutos);
    </script>
