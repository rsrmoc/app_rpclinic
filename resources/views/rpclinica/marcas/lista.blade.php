                        ...
                        <table class="display table dataTable table-striped">
                            ...
                            <tbody>
                                @foreach($marcas as $marca)
                                    <tr id="forma-{{ $marca->cd_marca }}">
                                        <th class="sorting_asc">{{ $marca->cd_marca }}</th>
                                        <td class="sorting">{{ $marca->nm_marca }}</td>
                                        <td class="sorting {{ $marca->sn_ativo == 'S' ? 'text-success' : 'text-danger' }}">
                                            <span class="{{ CSS_SN_ATIVO[$marca->sn_ativo] }}">{{ SN_ATIVO[$marca->sn_ativo] }}</span>
                                        </td>
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <a href="{{ route('marca.edit', ['marca' => $marca->cd_marca]) }}" class="btn btn-success">
                                                    <i class="fa fa-edit"></i>
                                                </a>

                                                <button onclick="delete_cadastro('{{ route('marca.delete', ['marca' => $marca->cd_marca]) }}', '#forma-{{ $marca->cd_marca }}')"
