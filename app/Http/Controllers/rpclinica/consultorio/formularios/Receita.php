...
class Receita extends Controller
{
    ...
    public function store(Request $request, Agendamento $agendamento)
    {
        try {
            ...
            $dados['cd_formulario'] = 'RECEITAS';

            $query = [
                'cd_agendamento' => $dados['cd_agendamento'],
                'cd_formulario' => 'RECEITAS'
            ];
    ...
    { 
        ...
        $historico = Agendamento::with(['profissional'])
        ->where('cd_paciente', $paciente->cd_paciente)
        ...
        $historico = $historico->where('oft_documentos.cd_formulario', 'RECEITAS')
        ->selectRaw("oft_documentos.*,agendamento.*,usuarios.*,oft_documentos.created_at created_data ")
        ->orderBy('dt_agenda', 'desc')->get(); 
   
        return  view('rpclinica.consultorio.formularios.oftalmologia.receita.modal', ['historico' => $historico]);
    }
