...
class Financeiro extends Controller
{
    ...
    public function index(Request $request) {
        ...
        return view("rpclinica.financeiro.lista", compact("categorias", "setores", "formasPagamento", "fornecedores", 
                                                 "contasBancaria", "marcas", "turmas", "eventos"));
        
    }

    public function saldoGeral($conta=null)
    { 
        $saldo= DocumentoBoleto::selectRaw("sum(case when tipo='despesa' then (vl_pagrec*(-1)) else vl_pagrec end) valor "); 
        if($conta){
            $saldo=$saldo->whereIn("cd_conta",$conta);
        }
        $saldo=$saldo->first(); 
        return (isset($saldo->valor) ? $saldo->valor : 0  );
    }
    ...
    public function jsonModal(Request $request)
    {
        ...
        $saldo = 0;
        $saldo = $this->saldoGeral( (($request->cd_conta) ? $request->cd_conta : null) );
          
        if($request->tp_pesquisa==false){
            $request['dt_final'] = null;
            $request['dt_inicial'] = null;
        }
        ...
        $vlReceita = array_sum(array_column($boletos->toArray()['data'],'vl_receita')); 
        $vlDespesa = array_sum(array_column($boletos->toArray()['data'],'vl_despesa'));
    ...
    public function quitarBoleto(Request $request,$tipo) {
  
        try {

            if($tipo=='RELACAO'){
                $request->validate([
                    "checkBoleto" => "required|array",  
                ]);
    
                foreach($request['checkBoleto'] as $boleto){
                    $documento=DocumentoBoleto::find($boleto);
                    if(($documento->tp_mov=='LC') || ($documento->situacao=='ABERTO')){ 
                        $vl_boleto=$documento->vl_boleto; 
                        $documento->update(['vl_pagrec'=>$vl_boleto,'dt_pagrec'=>date('Y-m-d'),
                        'cd_usuario_pagrec'=>$request->user()->cd_usuario, 
                        'data_pagrec'=>date('Y-m-d H:i'),'situacao'=>'QUITADO']);
                    }
                }
            }
            if($tipo=='RAPIDO'){
                $request->validate([
                    "cd_documento_boleto" => "required",  
                ]);
                $documento=DocumentoBoleto::find($request['cd_documento_boleto']);
                if(($documento->tp_mov=='LC') || ($documento->situacao=='ABERTO')){ 
                    $vl_boleto=$documento->vl_boleto; 
                    $documento->update(['vl_pagrec'=>$vl_boleto,'dt_pagrec'=>date('Y-m-d'),
                    'cd_usuario_pagrec'=>$request->user()->cd_usuario, 
    ...
    public function excluirBoleto(Request $request) {
        ...
        try {
            ...
            foreach($request['checkBoleto'] as $boleto){
                $documento=DocumentoBoleto::find($boleto);
                if(($documento->tp_mov=='LC')){ 
                    DocumentoBoleto::where('cd_documento_boleto', $documento->cd_documento_boleto)
                    ->delete();
                }
            }
    ...
    public function add(Request $request) {
        ...
        return view("rpclinica.financeiro.add", compact(
            ...
            "contasBancariaTransf","contasBancaria","turmas", "eventos","empresaUsuario"
        ));
    }
        ...
        try {
            ...
            if ($request->parcelar && is_array($request->parcelas) && count($request->parcelas) >= 1) {
                foreach($request->parcelas as $parcela) {
                    ...
                    $dataParcela["vl_boleto"] =   str_replace(',', '.',str_replace('.', '', $parcela["valor"]))  ;

                    $dataParcela["dt_pagrec"] = ($parcela["data_pagamento"]) ? $parcela["data_pagamento"] : null;
                    $dataParcela["vl_pagrec"] = ($parcela["valor_pago"]) ? str_replace(',', '.',str_replace('.', '', $parcela["valor_pago"])) : null;
                    ...
                    $dataParcela["data_pagrec"] = ($parcela["data_pagamento"]) ? date('Y-m-d H:i') : null;
                    ...
                    if(($parcela['valor_pago']>0) && (!$parcela['data_pagamento'])){
                        return response()->json(["message" =>  "Data de Pag./Rec. não informada" ], 500);
                    }
                    if(($parcela['valor_pago']<=0) && ($parcela['data_pagamento'])){
                        return response()->json(["message" =>  " Valor de Pag./Rec. não informado" ], 500); 
            ...
            $dataBoleto["vl_boleto"] = str_replace(',', '.',str_replace('.', '', $request->valor));   
            $dataBoleto["dt_pagrec"] = ($request->data_pagamento) ? $request->data_pagamento : null;
            $dataBoleto["vl_pagrec"] = ($request->valor_pago) ? str_replace(',', '.',str_replace('.', '', $request->valor_pago)) : null;
            ...
            $dataBoleto["data_pagrec"] = ($request->data_pagamento) ? date('Y-m-d H:i') : null;
   
            if(($request->valor_pago>0) && (!$request->data_pagamento)){
                return response()->json(["message" =>  "Data de Pag./Rec. não informada" ], 500);
            }
            if(($request->valor_pago<=0) && ($request->data_pagamento)){
                return response()->json(["message" =>  " Valor de Pag./Rec. não informado" ], 500); 
    ...
    public function edit(Request $request,DocumentoBoleto $documentoBoleto) {
        ...
        $documentoBoleto->load('conta','tab_fatura');
        $empresas = Empresa::where('cd_empresa',$request->user()->cd_empresa)->get(); 
        $categorias = Categoria::selectRaw("categoria.cd_empresa,categoria.cd_categoria,nm_categoria,case when sn_lancamento ='S' then false else true end lanc,cod_estrutural ")
        ->whereRaw("categoria.deleted_at is null")
        ->where('cd_empresa',$request->user()->cd_empresa)
        ->orderByRaw("RPAD( cast(replace(cod_estrutural, '.' , '') as unsigned integer),20,'0')") ->get();
        $setores = Setores::where('sn_ativo','S')->orderBy('nm_setor')->get();
        $formasPagamento = FormaPagamento::where('sn_ativo','S')->orderBy('nm_forma_pag')->get();
        $marcas = Marca::where('sn_ativo','S')->orderBy('nm_marca')->get();
        $fornecedores = Fornecedor::where('sn_ativo','S')->orderBy('nm_fornecedor')->get();
        $contasBancaria = ContaBancaria::where('sn_ativo','S')->orderBy('nm_conta')->get();
        $turmas = [];  
        $eventos = [];    
