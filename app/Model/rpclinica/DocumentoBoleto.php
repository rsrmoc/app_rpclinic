...
class DocumentoBoleto extends Model {
   // use SoftDeletes;
    ...
    public function tab_fatura() {
        return $this->hasOne(CartaoFatura::class, "cd_fatura", "cd_fatura_cartao");
    }
    ...
    public function scopePainelFinanceiro(Builder $query, $request): Builder
    {
        ...
        $query = $query->with(["empresa"=> function($q) use($request){ 
            $q->selectRaw("cd_empresa,nm_empresa");
        },"fornecedor","forma","conta"]) 
        ->leftJoin("categoria","categoria.cd_categoria","documento_boleto.cd_categoria");
  
        if($request->dt_inicial){ 
            $query->whereDate("dt_pesquisa", ">=", $request->dt_inicial);
        } 
        
        if($request->dt_final){ 
            $query->whereDate("dt_pesquisa", "<=", $request->dt_final);
        } 
         
        if ((!$request->dt_final) && (!$request->dt_inicial) ) {  
            $query->where( "dt_pesquisa", ">=",trim($request->ano.'-'.str_pad($request->mes , 2 , '0' , STR_PAD_LEFT).'-01') );
            $query->where("dt_pesquisa", "<=",trim($request->ano . '-' . str_pad($request->mes , 2 , '0' , STR_PAD_LEFT) . '-' . date("t", mktime(0,0,0, str_pad($request->mes , 2 , '0' , STR_PAD_LEFT),'01', $request->ano ))));
        }

        if($request->ds_boleto){ 
            $query =$query->where(DB::raw("upper(ds_boleto)"),"like",strtoupper($request->ds_boleto)); 
        }

        if($request->nr_documento){ 
            $query =$query->where( DB::raw("upper(doc_boleto)"),"like",strtoupper($request->nr_documento)); 
        }
        ...
        if($request->cd_setor){ 
            $query =$query->where("documento_boleto.cd_setor",$request->cd_setor); 
        } 
        if($request->cd_forma){ 
            $query =$query->where("documento_boleto.cd_forma",$request->cd_forma); 
        }  
        if($request->cd_conta){ 
            $query =$query->where("documento_boleto.cd_conta",$request->cd_conta); 
        }  
        if($request->cd_fornecedor){  
            $query =$query->where(DB::raw("ifnull(documento_boleto.cd_fornecedor,0)"),$request->cd_fornecedor);  
        }   
