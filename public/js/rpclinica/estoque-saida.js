...
/*!*************************************************!*\
  !*** ./resources/js/rpclinica/estoque-saida.js ***!
  \*************************************************/
Alpine.data('app', function () {
  return {
    ...
    init: function init() {
      ...
      if (typeof saidaProdutos !== 'undefined') {
        this.produtosEntrada = saidaProdutos;
      }
    },
    ...
    deleteEntradaProduto: function deleteEntradaProduto(indice) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          if (_this4.produtosEntrada[indice].cd_solic_saida) {
            axios.post("/rpclinica/estoque-saida-prod-delete/".concat(_this4.produtosEntrada[indice].cd_solic_saida)).then(function (res) {
              toastr['success']('Saída do produto excluída com sucesso!');

              _this4.produtosEntrada.splice(indice, 1);
            })["catch"](function (err) {
              return toastr['error']('Houve um erro ao excluir a saída do produto.');
            });
            return;
          }
