...
/*!***************************************************!*\
  !*** ./resources/js/rpclinica/estoque-entrada.js ***!
  \***************************************************/
    ...
    deleteEntradaProduto: function deleteEntradaProduto(indice) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          if (_this4.produtosEntrada[indice].cd_entrada_produto) {
            axios.post("/rpclinica/estoque-entrada-prod-delete/".concat(_this4.produtosEntrada[indice].cd_entrada_produto)).then(function (res) {
              toastr['success']('Entrada do produto excluída com sucesso!');

              _this4.produtosEntrada.splice(indice, 1);
            })["catch"](function (err) {
              return toastr['error']('Houve um erro ao excluir a entrada do produto.');
            });
            return;
          }
